...
| 问答路由（数值类 vs 文档类） | 数值/聚合类问题走 SQL/指标层，文档/解释类问题走 RAG，入口用一个路由判断进来的问题属于哪类 | 干净的分工：数字要精确计算、可机械追溯，语义检索算不准；文档/解释类内容才适合向量检索。两条链路互不污染 | 已决定 |
| 问答路由（数值类 vs 文档类） | 数值/聚合类问题走 SQL/指标层，文档/解释类问题走 RAG，入口用一个路由判断进来的问题属于哪类 | 干净的分工：数字要精确计算、可机械追溯，语义检索算不准；文档/解释类内容才适合向量检索。两条链路互不污染 | 已决定 |
...
| 口径歧义的分层处理 | 分两层：「同一数字多个副本」（原始/修正版/比较期重报）由数据层视图定死规则（取 filed 最新或带 frame 的官方点），用户不感知；「语义口径」（基本/稀释 EPS、财年/日历年、单季/累计）由问题解析层从提问中解析，解析不出时按预设默认值回答并在答案里写明所用口径，仅在无合理默认且结果差异大时反问 | 副本歧义属数据清洗，留给用户是灾难；语义口径才是用户真实意图，「默认值+回答透明」与时间口径默认值决策同一思路，该反问的场景评测集已覆盖 | 已决定 |
| 幻觉判定边界（WARNING vs ERROR） | 先用 prompt + 标注示例迭代打磨，不追求算法层面的精确边界 | 目前没有能做到近乎完美判断的公开方案，工程投入产出比更适合放在"未经证实"状态的体验设计上 | 已决定，持续迭代 |
| 检索反馈环（agentic 边界） | 检索采用有界反馈环：模型评估证据充分性并选择改写查询/扩大 k/放弃（≤3 轮、k 封顶、逐轮落日志、非法输出回退为一次性检索），但整体控制流仍由代码编排，不做自由运行的 agent loop | 一次性检索的失败模式是"库里有但没检到"（真实案例：中文提问对英文语料，目标块排第 13）；改写查询顺带解决跨语言措辞错位。决策点交给模型、控制流留在代码，保住金融场景的可审计性 | 已实现（Phase 1.5） |
